...
# mysterious_var is based to the scope. The 'New Surprise!' is local to the function,
# and the 'Surprise!' is global. The local mysterious_var shadows the global,
# creating a temporary local variable with the same name.
...
# If we append a value, we are not creating a local version of the variable, meaning
# that the method affects the global variable. If we assign it to a new variable, this will not happen.
def show_truth():
